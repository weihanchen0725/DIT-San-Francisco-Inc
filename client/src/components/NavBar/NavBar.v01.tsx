...
import NavBarData from "./NavBarData.v01.json";
...
const NavBar = () => {
  ...
  return (
    <nav
      ref={navRef}
      className="bg-white dark:bg-[#0a0a1a] shadow-sm dark:shadow-none border-b border-gray-100 dark:border-brand-navy-light relative transition-colors duration-200"
    >
                  ...
                  item.isActive && (
                    <React.Fragment key={item.name + index}>
                      ...
                      <Link
                        ...
                        className={
                          item.isDisabled
                            ? getDesktopLinkClasses({ color: "disabled" })
                            : getDesktopLinkClasses(item)
                        }
        ...
        <div className="px-4 py-3 space-y-1">
          {NavBarData.data.map((item, index) => (
            <React.Fragment key={item.name + index}>
              {index === NavBarData.data.length - 1 && (
                <div
                  key={`theme-switcher-${NavBarData.data.length}`}
                  className="flex items-center justify-between px-4 py-3 rounded-md text-brand-navy dark:text-gray-100 font-medium"
                >
              ...
              <Link
                ...
                href={item.isDisabled ? "#" : item.href}
                onClick={
                  item.isDisabled ? (e) => e.preventDefault() : closeMenu
                }
                className={
                  item.isDisabled
                    ? getMobileLinkClasses({ color: "disabled" })
                    : getMobileLinkClasses(item)
                }
              >
                {translateNavBar(item.name)}
              </Link>
